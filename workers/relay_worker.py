...
class RelayWorker():
	...
	def run(self): 
		...
		t.start()
		print('Relay Worker {tag}...\t\t\t\033[1;32m Running\033[0;0m'.format(**self.config))
		return t
	...
	def turnOn(self):
		...
		if self.relay_available.is_set():
			if not self.active:
				...
				variables.r.set(self.config['tag']+'_state', 1)
	...
	def turnOff(self):
		#Turn off voltage to flip off relay
		if self.relay_available.is_set():
			if self.active:
				GPIO.output(self.config['pin'], self.pin_state_off)
				message = {'event':'StateChanged', 'data':0}
				variables.r.delete(self.config['tag']+'_state')
				variables.r.publish(self.topic, json.dumps(message))
	...
	def work(self):
		...
		self.pubsub.close()
		print("Relay Worker {tag} Shutting Down...\t\033[1;32m Complete\033[0;0m".format(**self.config))